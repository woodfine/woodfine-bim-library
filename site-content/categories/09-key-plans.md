...
A Key Plan is a space, not a product — `IfcSpace` in the hierarchy above, the same category as a Tile or a Floor Plate, not the same category as an Object. It is the smallest spatial program worth leasing: real furniture placement, a three-part cross-section (Habitat / Magazine / Corridor), net leasable area, and accessibility compliance. We author each one from our own equipment programs; Key Plans nest into Tiles and Floor Plates without remainder, in both directions.

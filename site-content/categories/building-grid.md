...
property_sets:
---
Building Grid — the structural module and wall-thickness tolerances a floor plate's width adjusts within.

Structural module dimensions, wall and partition thicknesses, and the floor-plate-width tolerance bands (Green ±2.5ft, Yellow ±5ft, Red ±10ft) that drive the bidirectional width-adjustment logic — what happens when a target floor plate width doesn't land exactly on the Habitat/Magazine/Corridor arithmetic. Habitat and Corridor are locked by regulatory minima; Magazine is the only one of the three with no hard floor and yields first. See also Building Width Calculator for the zone-depth tokens these tolerances apply to.
